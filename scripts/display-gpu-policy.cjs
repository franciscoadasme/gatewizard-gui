...
function applyDisplayGpuEnv(env, opts = {}) {
  ...
  // Older builds saved a GPU crash here and then stayed on the CPU forever.
  // Drop that file. Software mode is only the current process (or an explicit env var).
  clearPersistedSafeMode(persistPath, fs)

  const safeRequested = env.GATEWIZARD_GPU_SAFE_MODE === '1'
  ...
  if (safeRequested) {
    policy = 'safe'
    reason = 'GATEWIZARD_GPU_SAFE_MODE'
...
# Older builds kept the CPU after one GPU crash. Never honor that file.
rm -f "\$gw_config/gpu-policy.json" 2>/dev/null || true
gw_wsl=0
