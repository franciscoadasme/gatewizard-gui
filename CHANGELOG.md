...
- **Status bar RAM:** The footer asks Windows through `powershell.exe` interop for installed RAM and available memory, then takes the larger total.
- **Visualize trajectory play (indexing):** Play stays disabled until the `.gwxyz` sidecar is finished **and** frames are in RAM or streaming is ready. A first click during a 2 GB copy looked locked because the UI thread was busy. Status shows Indexing, then Loading frames into RAM, then All frames in RAM. Space still explains why play is waiting.
- **Visualize trajectory play (large cache):** a ~2 GB DCD (about 97k atoms × 2000 frames) unpacks to ~2.2 GiB of float32 xyz. Loading that in one Electron IPC hit the ~2 GiB clone limit and aborted play. Sidecar reads now go in 256 MiB slices and yield between chunks so the window stays responsive. A failed cache read is shown next to the slider.
- **Visualize cartoon helix lighting:** the play-path skin first wrote ellipse-offset normals (a dark thread down each helix), then left frame-0 normals in place. Vertices followed the movie and hemisphere / directional lights shaded the stale normals, so the same helices flickered on the same frames. Play now rebuilds face-averaged normals after moving the ribbon (same as the original mesh).
- **Visualize trajectory play (ball-and-stick / licorice):** those styles now follow the play clock like vdW, cartoon, and tube. Licorice sticks were still reading the frame-0 atom objects, so caps moved and bonds did not.
- **Visualize trajectory surface:** Play does not remesh the organic surface (too heavy for 30 fps). Starting play with a visible Surface representation asks first; the last built skin stays on screen. Pause or export an image to rebuild it at the current frame.
- **Visualize trajectory lipids (vdW):** rotating the camera no longer hides a lipid cloud (for example resname PC) when the protein sits at the origin. Instanced spheres used a 1 Å bounding sphere, so the whole representation was culled as soon as the origin left the view.
- **Visualize trajectory play (points of all):** play no longer rebuilds the Visualize page every frame. The slider and GPU buffers follow a play clock, so a solvated points view stays in step with auto-generated cartoon / lipids.
- **Visualize RCSB download:** The download of PDB 5GOA failed because RCSB does not publish a legacy `.pdb` for it (105,072 atoms, past the 99,999 PDB serial limit) and the downloader only requested `.pdb`. The download now uses the PDB file when RCSB publishes one, and otherwise downloads the mmCIF and writes a PDB with hybrid-36 atom serials that the viewer can open. Residue numbers stay the author numbers when a new chain starts again at a low number. Opening a local `.cif` / `.mmcif` uses the same conversion.
- **Visualize trajectory play (points):** playing no longer rebuilds the point geometry every frame, which zeroed the color buffer and drew the atoms black. A cache read that still names the in-progress `.gwxyz.part` file also tries the finished `.gwxyz` after the extract renames it.
- **Visualize large structures:** a static entry over 80,000 atoms (5GOA) is drawn. A heavy style of trajectory `all` (vdW, ball-and-stick, cartoon) still stays empty above 80,000 atoms. The default points view of a trajectory is drawn, including a solvated PSF/DCD. Clearing the view used to assign a new empty list on every update, which exceeded Svelte’s update depth and left every representation blank.
- **Visualize bonds after mutation:** applying a rotamer reloads the new PDB with guessed bonds. A ball-and-stick or licorice view of one residue no longer stays disconnected when the parent structure was saved without CONECT records.
- **Visualize representation selection:** a ball-and-stick view of a single-bond residue (`protein and resid xx` i.e. after mutating that residue) no longer rewrites its bonds on every update. That loop exceeded Svelte’s update depth and crashed the window.
- **Visualize animation close:** leaving Animate no longer clones representation rows, which used to retrigger `/get-structure` on every card (stuck yellow spinners) and block Align / RMSD. Hidden representations stay hidden (eye state is written into keyframes from the playhead forward). `/get-structure` on a DCD/XTC + topology reads **frame 0 only** (PRMTOP/PSF have no coordinates by themselves).
- **Visualize Align / RMSD:** an applied alignment (selection, reference frame, and apply flag) is stored on the trajectory and in viewpoint / animation files, then restored after the sidecar is ready so the movie stays aligned.
- **Visualize Traj play:** wrapping waters/ions stay in the unit cell during Play (same as the frame slider and VMD). Min-image is only used to blend neighboring frames for Traj smooth, not accumulated across the movie.
- **Visualize trajectory overlays:** labels, measurements (canvas + panel values), in-progress picks, edit gizmo, center-view, and DOF focus follow atom coordinates as the trajectory plays. Representations can **Update each frame** so `byres` / `prop y < 0` clips keep whole residues as lipids move.
- **Visualize custom selections on a trajectory:** changing Other / MDAnalysis text no longer re-opens the DCD through `/get-structure` (that blocked the backend on large PRMTOP systems and left the yellow spinner up). Topology selections (`protein and resid 1`, `byres`, …) subset the already-loaded atoms in the client. New representations no longer attach every atom of a solvated trajectory (that cloned hundreds of thousands of objects and crashed the Electron renderer). Instance styles read packed xyz instead of cloning per frame.
- **Visualize interactive pick:** residue / atom / chain select uses camera-space depth so the front face wins. NDC z was flattened by the large far plane, so the first overlapping hit (often the hidden back residue) was kept.
...
- **Settings → Visualize:** **Load all frames into RAM up to** (0–16 GiB, 0 = always stream) and **Streaming cache** (64 MiB–2 GiB). Used the next time a trajectory is opened.
- **Visualize merge structures:** Ctrl+click two or more structures, then right-click **Merge structures** or use Tools → **Merge structures**. They are written, in panel order, into one new PDB. The originals stay loaded. The same chain letter is kept when the pieces are different kinds of residue (a protein and the ions on that chain). A second copy of the same protein, nucleic acid, ion, or ligand on that chain is refused.
- **Visualize Select on one structure:** Atom, residue, chain, and molecule selection, including the yellow outline and Rename chain, stay on the active structure. A second split or duplicate with the same chain id is no longer selected with it. Choosing a representation group first makes that structure the one Select edits.
- **Visualize duplicate structure:** the structure header **⧉** (and the right-click menu) copies the loaded structure, including the coordinates on screen, into a new entry with its own PDB. The original stays loaded. Each copy can be chain-renamed, superimposed, and saved with **Save PDB** on its own.
- **Visualize mutator:** Tools → **Mutate residue…** lists Dunbrack rotamers from pyMUT’s full `rotamers.lib` with probability and a relative heavy-atom van der Waals score, previews the side chain, and writes the chosen rotamer. The panel names the residue currently at that chain and number and the target amino acid in three-letter and one-letter notation (for example Ile (ILE, I) 60 chain A → Thr (THR, T)). Choosing a residue centers and zooms the camera on it and shows that residue as ball-and-stick. Backbone atoms are not moved. A trajectory uses frame 0, not the movie.
- **Visualize split chain and superimpose:** Tools → **Split chain…** copies one chain (ligands included) into a new structure and leaves the original loaded. Tools → **Superimpose…** fits one loaded protein chain onto another with Biotite sequence-guided superposition and moves every atom of the mobile structure. The reference stays fixed. Fewer than three sequence anchors leaves coordinates unchanged.
- **Visualize trajectories:** Open ▾ → **Trajectory…** loads one topology plus one or more DCD/XTC/TRR/NC/MDCRD (or multi-model PDB) files with per-file stride. A packed `.gwxyz` sidecar is built once (logical frames only); play loads all frames into RAM when that file is ≤ 4 GiB (1.5 GiB when the browser reports under 8 GB of memory), otherwise streams slices from disk. Play is a 30 fps clock (Space / ← →). Tools → **Align / RMSD…** Kabsch-fits a selection from the sidecar (same Analysis presets) and plots RMSD. Each representation has its own **Traj smooth** (0–8). Viewpoint v4 and animation v7 persist the paths, stride, playhead, and per-view smooth level. Auto-generate and cartoon SS use the topology (not the DCD).
...
- **Analysis RMSF:** **Prepare protein (unwrap + align)** (on by default; uses Align) makes the polymer whole across PBC before RMSF. Optional prep renum maps (`*_protonated_renum.txt` / `*_gatewizard_residue_mapping.txt` / `*gatewizard_to_original*`) remap topology residue ids → original PDB numbers (with set assignment and per-file enable); plots refresh immediately.
- **Analysis Radius of Gyration:** shows the same **Companion / Reference PDB** field as RMSD (shared path) so PSF/PRMTOP sets can reuse the RMSD reference for selection validation.
...
- **Display GPU policy (WSL / Linux / macOS):** before Chromium starts, WSL hosts that have `/dev/dxg` and `libd3d12.so` get `GALLIUM_DRIVER=d3d12` (Ubuntu 26.04 Mesa otherwise stays on llvmpipe). Native Linux and macOS are left on their normal stacks. The app never writes `MESA_D3D12_DEFAULT_ADAPTER_NAME`. If the GPU process dies, that session relaunches once on SwiftShader; the next launch tries the GPU again. A leftover `gpu-policy.json` is deleted. Overrides: `GATEWIZARD_GPU_SAFE_MODE`, `GATEWIZARD_GALLIUM_DRIVER`, existing `GALLIUM_DRIVER`.
...
- **Analysis energetic overlay / custom grid:** Energetic uses its own Overlay / Grid toolbar (not structural `compareLayout`). Overlay is one chart of visible sets × checked properties. Grid is the same mosaic chrome; each cell picks sets, draw order, title, and properties. Unedited seed is one cell per property with all visible sets. Old sessions: `by_property` → grid seed, `by_set` → one-set-per-cell seed.
- **Analysis energetic Run:** auto-detects the engine from each set’s logs, analyzes every native property found, remaps names to a canonical list, and unions checkboxes across mixed NAMD / OpenMM / GROMACS / Amber sets.
...
- **Visualize trajectory RAM cap:** load-all now accepts sidecars up to 4 GiB (was 3 GiB). Machines that report under 8 GB of memory still stay at 1.5 GiB.
- **Visualize trajectory play:** `.gwxyz` sidecar + load-all (≤ 4 GiB, or 1.5 GiB when the browser reports under 8 GB of memory) or disk slices; instance representations (VDW / ball-stick / licorice / points) read a packed xyz buffer instead of cloning atoms each frame; cartoon/tube keep the ribbon mesh and rewrite Cα / O positions while playing.
- **Visualize trajectory play (points):** each frame copies one coordinate block to the GPU and does not rebuild the bounding sphere. Labels, measurements, and depth-of-field are left alone while they are off. A streaming prefetch reads a span of cache frames in one file open.
- **Visualize trajectory play (cartoon / lipids):** the protein ribbon keeps its mesh and only moves Cα / carbonyl O each frame, so it follows the slider with the lipids. Traj smooth blends only the atoms that visible views use. vdW spheres write instance matrices as floats.
- **Visualize trajectory keyframes:** if the camera/view did not move since the previous keyframe, the new segment is **linear** (constant MD play). If the camera moved, the segment keeps the default **Smooth out**.
- **Visualize Align / RMSD:** two actions — **Calculate RMSD** (unaligned versus the reference frame; trajectory unchanged) and **Align trajectory** (Kabsch on the fit selection, aligned RMSD, same rotation/translation applied to every atom). Fit runs on sidecar mobile atoms (NumPy Kabsch); affines return as packed binary.
...
- **Visualize animation export:** “Preparing export…” no longer hangs — overwrite confirm runs before the overlay, the output folder is created in Electron (WSL/Windows path), and `frames/` is not fully listed (OneDrive). Trajectory frames are sought before each PNG, without invalidating the RAM cache.
- **Visualize Align / RMSD:** Kabsch used the transpose of the superposition rotation, so aligned backbone RMSD stayed several Å above Analysis (same selection). The fit now matches MDAnalysis ``rms.rmsd(..., superposition=True)``. Align trajectory now applies the fit to every atom in the viewer (not plot-only). Calculate RMSD is a separate action and does not move the movie. Traj smooth blends the **aligned** frames (alignment is the live trajectory), not the raw movie plus one leftover transform.
- **Visualize trajectory play (WSL Electron):** sidecar slice reads no longer open a Windows `C:\…` path from a Linux Electron process (`ENOENT` on `.gwxyz`). `fs:readBinarySlice` tries `/mnt/<drive>/…` and `C:\…`; play uses the Linux cache path first so frames stop skipping.
...
- **Analysis energetic Pub PNG:** Overlay and custom-grid mosaics use the same PlotSpec fields as structural (`cols` / `rows` / `cell_aspect` / per-panel `series_keys` and label visibility). Live SVG matches publication PNG.
